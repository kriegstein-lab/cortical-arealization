...
avg.exp <- lapply(markers.ct, function(markers) {
  ...
  avg.exp <- dp$data %>% rename(gene = "features.plot", 
                                group = "id") %>% 
              left_join(x %>% dplyr::select(-X)) %>% 
                rename(area = "cluster") %>% 
                   arrange(area, individual, gene)
...
ggsave(plot = h1, "../heatmaps_neocortex_DEgenes_celltype_x_area/heatmap_geneExp_markers_PFC_byIndividual_a.pdf", 
       device = "pdf", 
       height = 10, width = 25)
