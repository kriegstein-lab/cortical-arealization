seurat_to_h5ad <- function(seurat_object_path = NULL, seurat_object = NULL, file_path_out) {
  if(!is.null(seurat_object_path)) {
    seurat_object <- read_rds(seurat_object_path)
  }
  SeuratDisk::SaveH5Seurat(object = seurat_object, filename = file_path_out, overwrite = TRUE)
  SeuratDisk::Convert(source = file_path_out, dest = "h5ad", overwrite = TRUE)
  sms("Convert h5ad")
}
...
cleanMetadata <- function(seurat_object){
