...
goids_x %>% filter(name_1006 %>% str_detect("transcription|enhancer|methylation")) %>% distinct(hgnc_symbol, name_1006, .keep_all = TRUE) %>% add_count(name_1006) %>% arrange(desc(n)) %>% distinct(hgnc_symbol, .keep_all = TRUE) %>% dplyr::select(hgnc_symbol, name_1006, n, everything())
