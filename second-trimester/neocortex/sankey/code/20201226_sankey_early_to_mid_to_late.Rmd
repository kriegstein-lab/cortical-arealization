...
sankey_counts_neuron <- sankey_counts %>% dfilter(item1 %>% str_detect('neuron') & 
                                                        item2 %>% str_detect('neuron')) %>% dplyr::select(item1, item2, n, n_norm, genes)
...
sankey_counts_rg_ipc_neuron <- 
  # I think pairs` includes DE genes from all stages

sankey_counts %>% dfilter(item1 %>% str_detect('rg|ipc') & 
                    item2 %>% str_detect('ipc|neuron')) %>% 
...
write_tsv(sankey_counts_rg_to_neuron, file.path(out_dir, 'split_by_stage/between_celltypes/sankey_counts_rg_ipc_neuron_by_stages.tsv'))
