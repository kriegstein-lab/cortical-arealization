...
#' Dataframe providing information about the cluster call of nearest neighbours of cells within a cluster.
#'
#' Required columns:
#' "cl.from" = cluster_id of edge origin,
#' "cl.to" = cluster_id of edge destination,
#' "Freq" = , "cl.from.total" = total nr of neigbours (above threshold) from cluster of origin,
#' "cl.to.total" = total nr of neigbours (above threshold) from destination cluster,
#' "frac" = fraction of total edge outgoing.
#'
#' @param cl.center.df dataframe containing metadata and coordinates for plotting cluster centroids.
#'
#' Required columns:
#'"x" = x coordinate,
#'"y" = y coordinate,
#'"cl" = unique cluster id that should match "cl.to" and "cl.from" columns in knn.cl.df,
#' "cluster_color","size" = nr of cells in cluster
#'
...
plot_constellation <- function(knn.cl.df, cl.center.df, out.dir,
                                node.label = "cluster_id", exxageration = 2, curved = TRUE,
                                plot.parts=FALSE, plot.hull = NULL, plot.height=25, plot.width=25,
                                node.dodge=FALSE, label.size=2, max_size=10) {
 ...
 #############################
  ...
  g2 <- gridExtra::arrangeGrob(grobs=list(dot.size.legend,edge.width.legend,cl.center.legend),
                               layout_matrix=layout_legend)

  ggsave(file.path(out.dir, paste0(st,"constellation.pdf")),
  marrangeGrob(list(plot.all,g2), nrow = 1, ncol=1),width = plot.width, height = plot.height,
