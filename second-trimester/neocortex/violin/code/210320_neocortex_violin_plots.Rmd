...
settings <- tribble(
~slot_use, ~remove_0, ~dir_name,
"scale_data", FALSE, "scale_include_zero",
"scale_data", TRUE, "scale_remove_zero",
"data",       FALSE, "data_include_zero",
"data",       TRUE, "data_remove_zero")
...
violin_plots <- pmap(.l = settings[11,], 
...
patchwork::wrap_plots(violin_plots,ncol = 6, nrow = 3)
...
makeViolin <- function(genes, slot_use, remove_0 = FALSE, plots_per_page = 24, 
                       dir_name, scale_violin) {
                
   out_dir <- fs::dir_create(file.path("../out/allen", dir_name))
...
noise <- rnorm(n = length(expression) / 100000 )
...
plot <- df %>% ggplot(aes(x = !!group_by, y = expression, 
             colour = !!group_by, fill = !!group_by)) + 

  geom_violin(alpha = 0.2, scale = scale_violin, trim = trim_violin) +
  ggtitle(gene) +
...
if (remove_0 == TRUE | (remove_0 == FALSE & sum(expression > 0))) {
   plot <- plot + geom_jitter(alpha = 0.2,  size = 0.3)
}
