...
a <- loadData(rds_path = '../out_intermediate/ncx_exn_byCluster/dfs_and_objects_for_constellationPlot_ncx_exn_byCluster.rds') 
a$cl <- NULL
a$cl_numeric <- NULL
a$cl_center_df <- NULL
...
```{r}
a$cells_cl_df %>% dplyr::count(cell_type, annotation, celltype_area_2)
out_dir <- '~/Dropbox/ARK_lab/second-trimester/2nd Trimester Areas Paper/revisions/constellation/org_neuron_subtypes/subtypes_only'

source('../R/buildConstellation.R')

a_excit <- getExOnly(a)

result_all_stages <- a %>% buildConstellation(args_list = .,
                              name = 'all_stages',
                              groups.col = "celltype_area_2",
                              colors.use = cluster_colors,
                              run.knn = TRUE,
...
```{r}
out_dir <- '~/Dropbox/ARK_lab/second-trimester/2nd Trimester Areas Paper/revisions/plots_constellation/'
rds <- read_rds(file.path(/by_stages_no_subtypes/excitatory_only/pc50/20210315.1804_constellation_result_early.rds
```
